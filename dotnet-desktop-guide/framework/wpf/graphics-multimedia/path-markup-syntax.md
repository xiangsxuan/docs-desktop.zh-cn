...
|*startPoint*|<xref:System.Windows.Point?displayProperty=nameWithType><br /><br /> 新图形的起点。|  
 ...
 大写字母 `M` 指示 `startPoint` 是绝对值; 如果不存在，则为小写， `m` 表示与前一个点的 `startPoint` 偏移量，或 (0，0) 。 如果在移动命令之后列出多个点，可以绘制一条连接到这些点的直线，尽管已指定了直线命令。  
...
大写字母 `L` 指示 `endPoint` 是绝对值; 如果不存在，则为小写， `l` 表示与前一个点的 `endPoint` 偏移量，或 (0，0) 。
...
|语法|  
|------------|  
|`H`  *x*<br /><br /> - 或 -<br /><br /> `h`  *x-blade*|  
  
|术语|说明|  
|----------|-----------------|  
|*x*|<xref:System.Double?displayProperty=nameWithType><br /><br /> 线条终点的 x 坐标。|  
  
大写字母 `H` 指示 `x` 是绝对值; 如果不存在，则为小写， `h` 表示与前一个点的 `x` 偏移量，或 (0，0) 。
...
大写字母 `V` 指示 `y` 是绝对值; 如果不存在，则为小写， `v` 表示与前一个点的 `y` 偏移量，或 (0，0) 。  

### <a name="cubic-bezier-curve-command"></a>三次贝塞尔曲线命令  
 使用两个指定控制点 (`controlPoint` 1 和 2) 在当前点和指定的终点之间创建三次方贝塞尔曲线 `controlPoint` 。 `C 100,200 200,400 300,200` 是有效曲线命令的示例。  
