...
核心架构定义的主 XML 结构 <xref:System.Windows.Annotations.Annotation> 。  核心架构中定义的大多数 XML 元素对应于命名空间中的类型 <xref:System.Windows.Annotations> 。  核心架构公开应用程序可在其中添加自己的 XML 数据的三个扩展点。  这些扩展点包括 <xref:System.Windows.Annotations.Annotation.Authors%2A> 、 <xref:System.Windows.Annotations.ContentLocatorPart> 和 "内容"。   (内容元素以列表的形式提供 <xref:System.Xml.XmlElement> 。 ) 

本主题中所述的基本架构定义了 <xref:System.Windows.Annotations.Annotation.Authors%2A> <xref:System.Windows.Annotations.ContentLocatorPart> (WPF) 版本的初始 Windows Presentation Foundation 附带的、和内容类型的扩展。

<a name="CoreSchema"></a>

## <a name="annotations-xml-core-schema"></a>批注 XML 核心架构

批注 XML 核心架构定义用于存储对象的 XML 结构 <xref:System.Windows.Annotations.Annotation> 。
...
基本架构为核心架构中定义的三个抽象元素（ <xref:System.Windows.Annotations.Annotation.Authors%2A> 、和）定义 XML 结构 <xref:System.Windows.Annotations.ContentLocatorPart> <xref:System.Windows.Annotations.AnnotationResource.Contents%2A> 。
...
下面的 XML 显示了一个批注的输出， <xref:System.Windows.Annotations.Storage.XmlStreamStore> 以及一个包含三个批注的示例文件（一个突出显示、文本粘滞便笺和一个墨迹便笺）的组织。
