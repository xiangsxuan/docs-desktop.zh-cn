...
如果应用程序是针对某种显示分辨率设计的，会发生类似情况。 最常见的显示分辨率为每英寸96点 (DPI) ，这等于100% 的显示比例，但更高) 的分辨率会显示支持125%、150%、200% (，这二者会越来越常见。 如果不进行调整，针对某个分辨率设计的应用程序（特别是基于图形的应用程序）在其他分辨率上运行时就会显示太大或太小。
