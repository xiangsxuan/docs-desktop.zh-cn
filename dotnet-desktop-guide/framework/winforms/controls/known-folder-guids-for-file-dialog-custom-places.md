...
向 <xref:System.Guid> 集合中添加文件夹时，可以使用指定 Windows Vista 已知文件夹 <xref:System.Windows.Forms.FileDialog.CustomPlaces%2A> 。 已知文件夹 Guid 不区分大小写，并且在 Windows SDK 的 Knownfolders.h 文件中定义。  
