...
如果 <xref:System.Windows.Forms.DataGridView> 控件设置为基于数据源中的数据自动生成列，则可以选择性地省略某些列。 可以通过对集合调用方法来执行此操作 <xref:System.Windows.Forms.DataGridViewColumnCollection.Remove%2A> <xref:System.Windows.Forms.DataGridView.Columns%2A> 。 或者，您可以通过将 <xref:System.Windows.Forms.DataGridViewColumn.Visible%2A> 属性设置为来隐藏列 `false` 。 如果希望在某些情况下显示隐藏的列，或者需要访问列中的数据而不显示列，此方法非常有用。  
  
### <a name="to-remove-autogenerated-columns"></a>删除自动生成的列  
  
- 对 <xref:System.Windows.Forms.DataGridViewColumnCollection.Remove%2A> 集合调用方法 <xref:System.Windows.Forms.DataGridView.Columns%2A> 。  
...
- 一个 <xref:System.Windows.Forms.DataGridView> 名为的控件 `dataGridView1` ，该控件绑定到一个表中，该表包含 `Fax` 和 `CustomerID` 列，如 `Customers` Northwind 示例数据库中的表。  
