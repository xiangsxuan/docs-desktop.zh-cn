...
1. 使用 <xref:System.Windows.Forms.CommonDialog.ShowDialog%2A> 方法可显示对话框，从而指定要使用的 <xref:System.Drawing.Printing.PrintDocument> 。  
     ...
     该示例假设窗体具有一个 <xref:System.Windows.Forms.Button> 控件、一个 <xref:System.Drawing.Printing.PrintDocument> 名为的组件 `myDocument` 和一个 <xref:System.Windows.Forms.PageSetupDialog> 组件。  
