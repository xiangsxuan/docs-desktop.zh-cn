...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>组件 " \<component name> " 未调用容器。请在其构造函数中添加 ( # A1

这是你刚加载或放置在窗体上的组件中的错误。 它指示组件未将自身添加到其容器控件， (是) 其他控件还是窗体。 设计器将继续工作，但在运行时组件可能会出现问题。
...
出现此错误是因为成员变量已被声明两次 (例如， `Button1` 在代码) 中声明了两个名为的控件。 名称在继承的窗体中必须是唯一的。 此外，名称不能仅区分大小写。
...
不能将控件或组件添加到其自身。 出现这种情况的另一种情况是，在窗体的 InitializeComponent 方法中存在代码 (例如，创建 Form1 的另一个实例的 Form1) 。
